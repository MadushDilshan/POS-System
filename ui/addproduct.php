...
if (isset($_POST['btnsave'])) {
  try {
    ...
    $retailsaleprice = $_POST['txtretailsaleprice'];
    $wholesaleprice = $_POST['txtwholesaleprice'];

    if ($purchaseprice > $retailsaleprice) {
...
//      echo '<script type="text/javascript">window.location.href="addproduct.php";</script>';
    ...
    if (empty($barcode)) {
      ...
      if ($query->rowCount() == 0) { // No matching product found by name
        ...
        $insertStock = $pdo->prepare("INSERT INTO Product_Stock (pid, stock, purchaseprice, retailsaleprice, wholesaleprice) VALUES (:pid, :stock, :purchaseprice, :retailsaleprice, :wholesaleprice)");
        $insertStock->bindParam(':pid', $pid);
        $insertStock->bindParam(':stock', $stock);
        $insertStock->bindParam(':purchaseprice', $purchaseprice);
        $insertStock->bindParam(':retailsaleprice', $retailsaleprice);
        $insertStock->bindParam(':wholesaleprice', $wholesaleprice);
        $insertStock->execute();

        // Insert into incoming_stock after updating Product_Stock (Barcode not given)
        $insertIncomingStock = $pdo->prepare("INSERT INTO incoming_stock (pid, stock, purchaseprice, retailsaleprice, wholesaleprice, date_time) VALUES (:pid, :added_stock, :purchaseprice, :retailsaleprice, :wholesaleprice, NOW())");
        ...
        $insertIncomingStock->bindParam(':retailsaleprice', $retailsaleprice);
        $insertIncomingStock->bindParam(':wholesaleprice', $wholesaleprice);
      ...
      if ($checkBarcode->rowCount() == 0) { // No matching barcode found
        ...
        $checkStock = $pdo->prepare("SELECT * FROM Product_Stock WHERE pid = :pid AND purchaseprice = :purchaseprice AND retailsaleprice = :retailsaleprice AND wholesaleprice = :wholesaleprice");
        $checkStock->bindParam(':pid', $pid);
        $checkStock->bindParam(':purchaseprice', $purchaseprice);
        $checkStock->bindParam(':retailsaleprice', $retailsaleprice);
        $checkStock->bindParam(':wholesaleprice', $wholesaleprice);
        $checkStock->execute();
                ...
                <div class="row">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                      ...
                      <input type="number" min="1" step="any" class="form-control" placeholder="Enter Retail Selling Price"
                             name="txtretailsaleprice" autocomplete="off" required oninput="validatePrices()"
                             onblur="retailsalePriceOnBlur()">
                    </div>

                    <div class="form-group">
                      <label>Whole Selling Price</label>
                      <input type="number" min="1" step="any" class="form-control" placeholder="Enter Whole Selling Price"
                             name="txtwholesaleprice" autocomplete="off" required oninput="validatePrices()"
                             onblur="wholesalePriceOnBlur()">
                    </div>
...
<script type="text/javascript">
  function validatePrices() {
    ...
    var retailsalePrice = document.getElementsByName('txtretailsaleprice')[0].value;
    var wholesalePrice = document.getElementsByName('txtwholesaleprice')[0].value;

    var retailsalePriceField = document.getElementsByName('txtretailsaleprice')[0];
    var wholesalePriceField = document.getElementsByName('txtwholesaleprice')[0];

    // Remove error class first
    retailsalePriceField.classList.remove('error');
    wholesalePriceField.classList.remove('error');

    if (parseFloat(purchasePrice) > parseFloat(retailsalePrice)) {
      // Apply error class
      retailsalePriceField.classList.add('error');
    }

    if (parseFloat(purchasePrice) > parseFloat(wholesalePrice) || parseFloat(wholesalePrice)>parseFloat(retailsalePrice)) {
      // Apply error class
      wholesalePriceField.classList.add('error');
    }
  }
  ...
  function retailsalePriceOnBlur() {
    if (document.getElementsByName('txtretailsaleprice')[0].value <= 0) {
      document.getElementsByName('txtretailsaleprice')[0].value = 1;
    }
  }

  function wholesalePriceOnBlur() {
    if (document.getElementsByName('txtwholesaleprice')[0].value <= 0) {
      document.getElementsByName('txtwholesaleprice')[0].value = 1;
    }
  }
...
<script>
  document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const retailsellingPriceInput = document.getElementsByName('txtretailsaleprice')[0];
    const wholesellingPriceInput = document.getElementsByName('txtwholesaleprice')[0];
    ...
    function validateInputs() {
      ...
      const retailsellingPrice = parseFloat(retailsellingPriceInput.value);
      const wholesellingPrice = parseFloat(wholesellingPriceInput.value);

      console.log(stock, purchasePrice, retailsellingPrice, wholesellingPrice);

      // Check if any of the values are negative or purchase price is greater than selling price
      if (isNaN(stock) || stock < 0 || isNaN(purchasePrice) || purchasePrice < 0 || isNaN(retailsellingPrice) || retailsellingPrice < 0 || purchasePrice > retailsellingPrice || isNaN(wholesellingPrice) || wholesellingPrice < 0 || purchasePrice > wholesellingPrice) {
    ...
    retailsellingPriceInput.addEventListener('input', validateInputs);
    wholesellingPriceInput.addEventListener('input', validateInputs);
