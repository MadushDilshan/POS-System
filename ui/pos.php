...
<style>
    ...
    tr {
      animation: colorChange 4s linear forwards; /* Play the animation once and stay in the final state */
    }
...
<script>
  ...
  function handleEnter(event, stock_id) {
    if (event.key === "Enter") {
      ...
      if (!Number.isInteger(qtyValue) || qtyValue < 1) {
        qtyInput.value = 0; // Set it to 1 if it's a float or less than 1
      }
  ...
  function handleBlur(event, stock_id) {
    ...
    // Check if the entered value is a float or less than 1
    if (!Number.isInteger(qtyValue) || qtyValue < 1) {
      $('#qty_id' + stock_id).val(0).trigger('change')
    }
  ...
  $(function () {
          ...
          $.each(response.data, function (index, item) {
            ...
            if (jQuery.inArray(stock_id, productarr) !== -1) {
              ...
              if (stock_quantity > parseInt($('#qty_id' + stock_id).val())) {
                let actualqty = parseInt($('#qty_id' + stock_id).val());
                $('#qty_id' + stock_id).val(actualqty);
                // let saleprice = parseInt(actualqty) * item.retailsaleprice;
              ...
              saleprice[stock_id] = {stock_id:stock_id,retailsaleprice:item.retailsaleprice,wholesaleprice:item.wholesaleprice};
              addrow(item.pid, item.product, item.retailsaleprice,item.wholesaleprice, item.stock, item.barcode, stock_id);
              ...
              $.each(response.data, function (index, item) {
                ...
                if (jQuery.inArray(stock_id, productarr) !== -1) {
                  ...
                  if (stock_quantity > parseInt($('#qty_id' + stock_id).val())) {
                    let actualqty = parseInt($('#qty_id' + stock_id).val());
                    $('#qty_id' + stock_id).val(actualqty);
                    // let saleprice = parseInt(actualqty) * item.wholesaleprice;
                    // $('#total_raw_price_id' + stock_id).html(saleprice);
                    // $('.totalamt').val(saleprice);
    ...
    function addrow(pid, product, retailsaleprice,wholesaleprice, stock, barcode, stock_id) {
      let tr =
        '<tr>' +
        ...
        '<td style="text-align:left; vertical-align:middle; font-size:17px;"><span class="badge badge-warning price" name="price_arr[]" id="price_id' + stock_id + '">' + retailsaleprice + '</span><input type="hidden" class="form-control sale_price_id" name="sale_price_id_arr[]" id="sale_price_id' + stock_id + '"></td>' +
        ...
        '<td style="text-align:left; vertical-align:middle; font-size:17px;"><span class="badge badge-success totalamt" name="netamt_arr[]" id="total_raw_price_id' + stock_id + '">' + "0" + '</span><input type="hidden" class="form-control saleprice" name="retailsaleprice_arr[]" id="retailsaleprice_idd' + stock_id + '"></td>' +
        ...
        '<td><input style="width: 80px" type="checkbox" class="form-control changeprice" data-id="' + stock_id + '" name="change_arr[]" id="change_id' + stock_id + '" size="1" min="1" onkeydown="handleEnter(event, ' + stock_id + ')" onblur="handleBlur(event, ' + stock_id + ')"></td>' +
        '</tr>';
  ...
  $("#itemtable").on("keyup change", ".qty", "keyup change", function () {
    ...
    if ((quantity.val() - 0) > (tr.find(".stock_qty").val() - 0)) {
      ...
      tr.find(".sale_price_id").val(tr.find(".price").text());
      calculate();
      updateOrderButtonState();
      checkprice();
...
function checkprice() {
  ...
  $("#itemtable").on("keyup change", ".changeprice", "keyup change", function () {
    ...
    let wholesaleprice = saleprice[stock_id].wholesaleprice;
    let retailprice = saleprice[stock_id].retailsaleprice;
    
    // Check if the checkbox is checked
    if ($(this).prop('checked')) {
        // Update the values in the third and fourth columns
        
        console.log(wholesaleprice);
        row.find(".price").text(wholesaleprice);
        row.find(".totalamt").text(row.find(".qty").val() * wholesaleprice);
        row.find(".saleprice").val(row.find(".qty").val() * wholesaleprice);
        row.find(".sale_price_id").val(wholesaleprice);
        calculate();
        updateOrderButtonState();
        checkprice();
        
    } else {
        // If the checkbox is unchecked, revert to the original values
        console.log(retailprice);
        row.find(".price").text(retailprice);
        row.find(".totalamt").text(row.find(".qty").val()* retailprice);
        row.find(".saleprice").val(row.find(".qty").val() * retailprice);
        row.find(".sale_price_id").val(retailprice);
        calculate();
        updateOrderButtonState();
        checkprice();
    }
